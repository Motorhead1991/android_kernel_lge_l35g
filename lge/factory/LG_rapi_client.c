...
//	arg.input = NULL;
	arg.out_len_valid = 1;
	arg.output_valid = 1;
	arg.output_size = sizeof(lock_state);
	...
	if(ret.output == NULL || ret.out_len == NULL)
	{
		printk(KERN_ERR "oem_rapi_client_streaming_function: output is null\n");
		return -1;
	}
...
int lg_usb_lock_code_is_valid(char *key, int size)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	...
	arg.event = LG_FW_GET_USB_LOCK_CODE_VALID;
	arg.cb_func = NULL;
	arg.handle = (void*) 0;
	arg.in_len = size;
	arg.input = key;
...
EXPORT_SYMBOL(lg_usb_lock_code_is_valid);
#endif
