...
#define CHG_UPDATE_TIME msecs_to_jiffies(2000)
...
#define ep0in  ept[16]
	...
	struct wake_lock    charger_update_wlock;
};
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	if (temp == USB_CHG_TYPE__WALLCHARGER) {
		...
		if (wake_lock_active(&ui->charger_update_wlock))
			wake_unlock(&ui->charger_update_wlock);
 		wake_lock_timeout(&ui->charger_update_wlock, CHG_UPDATE_TIME);
	}
}
...
static int msm72k_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&ui->charger_update_wlock,
	...
	if (retval) {
		...
		wake_lock_destroy(&ui->charger_update_wlock);
		return usb_free(ui, retval);
	}
