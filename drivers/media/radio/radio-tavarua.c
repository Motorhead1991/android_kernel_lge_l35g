...
{
	FMDERR("%s: Reading the INTSTAT Registers\n", __func__);
	...
	if (radio->registers[STATUS_REG3] & TRANSFER) {
		FMDERR("%s: XFR Interrupt happened but GPIO did not toggle\n",
			__func__);
		tavarua_read_registers(radio, XFRCTRL, XFR_REG_NUM+1);
		FMDERR("%s: XFRCTRL IS: %x\n", __func__,
			radio->registers[XFRCTRL]);
	} else
		FMDERR("%s: TRANSFER interrupt did not happen!\n", __func__);
}
/*=============================================================================
				...
				int req_region)
{
	...
	default:
		...
		xfr_buf[3] = band_high & 0xFF;
              for(loop_sc=4; loop_sc<XFR_REG_NUM; loop_sc++)
              {
...
static int tavarua_fops_release(struct file *file)
{
    ...
    /*BEGIN: Modified Code*/ 
    FMDERR("Wait for the READY event to be triggered before un-registering the Interrupt handler");
    msleep(TAVARUA_DELAY*10); 
    /*END: Modified Code*/ 
		...
		struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_PRIVATE_TAVARUA_LP_MODE:
		...
		if (ctrl->value == radio->lp_mode) {
			if (ctrl->value) {
				FMDERR("%s: System already in LOW Power Mode\n",
					__func__);
				break;
			} else {
				FMDERR("%s: System already in Normal Mode\n",
					__func__);
				break;
			}
		}
...
/* LGE_CHANGE_S : E0 sungmin1217.kim@lge.com [2012-01-13]
	Reson : <Qualcomm Patch> Make BT not to sleep BT/FM chipset when FM Radio is running.
*/
		if (!users) {	//if (users)
/* LGE_CHANGE_E : E0 sungmin1217.kim@lge.com@lge.com [2012-01-13] */
			retval = tavarua_disable_interrupts(radio);
			if (retval < 0) {
